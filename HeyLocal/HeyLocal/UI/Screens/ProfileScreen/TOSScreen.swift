...
struct TOSScreen: View {
	...
	**제 1 조 (목적)**
	본 약관은 <SWM-TGT>이(가) 운영하는 모바일 어플리케이션(현지야)의 제반 서비스의 이용조건 및 절차에 관한 사항 및 기타 필요한 사항을 규정함을 목적으로 한다.
	...
	**제 7 조 (회사의 의무)**
	①회사는 서비스를 안정적이고 지속적으로 운영할 의무가 있다.
	...
	**제 8 조 (이용자의 의무)**
	①이용자는 본 약관 및 회사의 공지사항, 서비스 이용안내 등을 숙지하고 준수해야 하며 기타 회사의 업무에 방해되는 행위를 해서는 안된다.
	②이용자는 회사의 사전 승인 없이 본 서비스를 이용해 어떠한 영리행위도 할 수 없다.
	③이용자는 본 서비스를 통해 얻는 정보를 회사의 사전 승낙 없이 복사, 복제, 변경, 번역, 출판, 방송 및 기타의 방법으로 사용하거나 이를 타인에게 제공할 수 없다.
